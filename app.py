...
def get_profile():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return render_template('user_info.html',user_profile=user_profile)
...
def check_permission():
    allowed_actions = {
        'doctor': ['view_patients', 'view_appointments','update_patient_status' ],
        'nurse': ['create_appointments','view_patients']
    }
    if request.method == 'POST':  # Check if request is POST
        role = request.form.get('role')  
        permission= request.form.get('permission')
        # if permission in allowed_actions.get(role, []):
        #     return jsonify({'allowed': True})
        # else:
        #     return jsonify({'allowed': False, 'message': 'Unauthorized'})
        if permission in allowed_actions.get(role, []):
            return render_template('permission.html', allowed=True)
        else:
            return render_template('permission.html', allowed=False)
...
@app.route('/logout')
def logout():
    logout_url = 'http://localhost:8080/realms/myorg/protocol/openid-connect/logout'
    # You might need to add additional parameters to the logout URL based on your Keycloak configuration
